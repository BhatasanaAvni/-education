...
const CountUpAnimation = () => {
  ...
  // State to control countdown behavior
  const [countdown, setCountdown] = useState(false);

  // Handle countdown behavior when section is in view
  React.useEffect(() => {
    if (inView) {
      setCountdown(true); // Start countdown when inView is true
    }
  }, [inView]);
                ...
                <div className="flex-grow-1 mt-6">
                  <h1 className="text-white text-6xl">
                    ...
                    <CountUp
                      ...
                      separator=","
                      suffix="+"
                      onEnd={() => setCountdown(false)} // Stop once the target is reached
                      useEasing={true} // Use easing for smooth animation
                    />
                  </h1>
                  <p className="text-white uppercase mt-4 font-bold text-42l">
                    {counter.label}
